...
CommandLine:
    python -m netharn.examples.cifar --xpu=0 --arch=resnet50
    python -m netharn.examples.cifar --xpu=0 --arch=wrn_22 --lr=0.003 --schedule=onecycle --optim=adamw
    python -m netharn.examples.cifar --xpu=1,2,3 --arch=wrn_22 --lr=0.003 --schedule=onecycle --optim=adamw --batch_size=1800
    python -m netharn.examples.cifar --xpu=1,2 --arch=resnet50 --lr=0.003 --schedule=onecycle --optim=adamw
...
class CIFAR_FitHarn(nh.FitHarn):
    ...
    def prepare_batch(harn, raw_batch):
        """
        By default this function does nothing, but if we specify it we
        can transform the batch after it is generated by the dataloader.
        """
        inputs, labels = harn.xpu.move(raw_batch)
        batch = {
            'input': inputs,
            'label': labels,
        }
